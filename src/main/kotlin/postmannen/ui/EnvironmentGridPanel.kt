...
// This constraint is tighter here than it was in the old popup, since this
// panel only gets roughly half the terminal's width, not the whole screen.
private const val VALUE_COLUMN_WIDTH = 18
private const val KEY_COLUMN_WIDTH = 18

class EnvironmentGridPanel(
    ...
    fun takeFocus() {
        rows.firstOrNull()?.keyBox?.takeFocus()
    }
    ...
    // Ctrl+D never reach the main window's onUnhandledInput handler while any
    // TextBox in this grid has focus (which is almost always). Both TextBox
    // subclasses below call this first and only fall through to the default
    // text-insert behavior if it returns null. App.kt's window listener also
    // calls this directly, covering the case where a CheckBox (which doesn't
    // consume arbitrary character keys) has focus.
    fun handleAddDeleteShortcut(key: KeyStroke): Interactable.Result? {
    ...
    private fun removeRow(row: Row) {
        removeComponent(row.keyBox)
        row.cellsByUid.values.forEach { (checkBox, valueBox) ->
            removeComponent(checkBox)
            removeComponent(valueBox)
        }
        rows.remove(row)
    }
